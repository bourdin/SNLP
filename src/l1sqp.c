...
L1SQP L1SQPNew(NLP nlp) {
    ...
    a->printFunctionGradient = NO;      // default: NO
    a->printConstraintGradient = NO;    // default: NO
    a->simpleLineSearch = NO;           // default: NO
    ...
    a->print_fun_grad = NO; // print the function gradients at each step
    a->print_con_grad = NO; // print the constraint gradients at each step
    a->simple_line_search = NO; // implement a simple function reduction line search
...
int L1SQPcheckNLP(L1SQP a, Vector x0, Vector lambda0, Vector mu0) {
    ...
    if ((a->printFunctionGradient != YES) && (a->printFunctionGradient != NO)) {
        a->print_fun_grad = NO;
    } else {
        a->print_fun_grad = a->printFunctionGradient;
    }

    if ((a->printConstraintGradient != YES) && (a->printConstraintGradient != NO)) {
        a->print_con_grad = NO;
    } else {
        a->print_con_grad = a->printConstraintGradient;
    }

    if ((a->simpleLineSearch != YES) && (a->simpleLineSearch != NO)) {
        a->simple_line_search = NO;
    } else {
        a->simple_line_search = a->simpleLineSearch;
    }
...
void L1SQPlineSearch(L1SQP a) {
    ...
    if (a->print_fun_grad == YES) {
        printf("df:\n");
        VectorPrint(a->d);
    }

    if ((m > 0) && (a->print_con_grad == YES)) {
        printf("dh:\n");
        MatrixPrint(a->dh);
    }
    
    if ((p > 0) && (a->print_con_grad == YES)) {
        printf("dg:\n");
        MatrixPrint(a->dg);
    }
    ...
    for (ls_iter = 0; ls_iter < MAX_LS_ITER; ls_iter++) {
        ...
        fac1 = a->theta0;
        if (a->simple_line_search == NO) {
            fac1 += a->alpha * SIGMA * a->Dtheta;
        }
